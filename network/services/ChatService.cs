...
public static class ChatService
{
    ...
    public static void HandleChatMessageRequest(ENetPacketPeer peer, byte[] data)
    {
        ...
        var info = new ChatMessageInfo(chatMessageRequest.Channel, chatMessageRequest.Text, peerID);
    ...
    public static bool ValidateChatMessageRequest(ChatMessageInfo info)
    {
        if (info.Text.Length > MAX_CHAT_MESSAGE_CHARACTERS)
        {
            info.Text = info.Text.Substring(0, MAX_CHAT_MESSAGE_CHARACTERS);
        }
        return true; // no other validation for now
    }
